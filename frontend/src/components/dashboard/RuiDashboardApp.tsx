 ...
 */
export const RuiDashboardApp: React.FC<RuiDashboardAppProps> = ({
    initialPath,
}) => {
    ...
    const getNavIdFromPath = (path: string): string => {
        ...
        if (
            ...
            path.startsWith("/en/registrations")
        ) {
            return "inscripciones";
        }
        // Control de acceso
        if (
            path.startsWith("/control-acceso") ||
            path.startsWith("/en/access-control")
        ) {
        ...
        // Padrón CIP
        if (
            path.startsWith("/padron-cip") ||
            path.startsWith("/en/cip-registry")
        ) {
            return "padron-cip";
        }
        // Configuración
    ...
    const renderContent = () => {
        ...
        if (matchesRoute(["/eventos/nuevo", "/en/events/new"])) {
            return (
                <SectionPlaceholder
                    title={t("sections.create_event", "Crear Evento")}
        ...
        const organizerDetailMatch = activePath.match(
            /^\/(organizadores|en\/organizers)\/([a-zA-Z0-9-]+)$/
        );
        ...
        if (startsWithRoute(["/control-acceso", "/en/access-control"])) {
            return (
                <SectionPlaceholder
                    title={t("sections.access_control")}
                    description={t("sections.coming_soon")}
                />
            );
        }
        ...
        if (startsWithRoute(["/finanzas", "/en/finance"])) {
            return (
                <SectionPlaceholder
                    title={t("sections.payments_reports")}
                    description={t("sections.coming_soon")}
                />
            );
        }
        ...
        // Detalle de usuario
        const userDetailMatch = activePath.match(
            /^\/(usuarios|en\/users)\/([a-zA-Z0-9-]+)$/
        );
        if (
            userDetailMatch &&
            userDetailMatch[2] !== "nuevo" &&
            userDetailMatch[2] !== "new"
        ) {
            const userId = userDetailMatch[2];
            return (
                <UserDetailView userId={userId} onNavigate={handleNavigate} />
            );
        }
        // Lista de usuarios
